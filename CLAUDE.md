...
| --- | --- | --- |
| `nemotron-predict-client` | `nemotron_predict` | The client. Pure Python, no heavy dependencies. |
| `nemotron-predict-connectors` | `nemotron_predict_connectors` | Reads source tables from sqlite, duckdb, Snowflake, Databricks, S3. |
...
`PredictClient` dispatches to whichever model you ask for:
...
**Drive the relational model only through `PredictClient`.** The driver keeps a
...
`PredictError` or a subclass carrying a stable `code`: `MISSING_EXTRA`,
