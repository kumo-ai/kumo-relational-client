...
def contract_compliance_regressions() -> None:
    ...
    response = requests.post(
        BASE_URL + '/v1/predictions', json=body, timeout=30
    )
    ...
    timestamp_body = {
        ...
        'context': {
            'instance_table': {
                ...
                'columns': ['ts', 'label'],
                'rows': [
                    ['2025-01-01T00:00:00.000Z', 'a'],
                    ['2025-01-02T00:00:00.000Z', 'b'],
                ],
            }
        },
        'predict': {
            'instance_table': {
                'format': 'arrays',
                'columns': ['ts'],
                'rows': [['2025-01-03T00:00:00.000Z']],
            }
        },
        'output': {'fields': ['prediction']},
    }
    ts_response = requests.post(
        BASE_URL + '/v1/predictions', json=timestamp_body, timeout=30
    )
    check(
    ...
    malformed_response = requests.post(
        BASE_URL + '/v1/predictions', json=malformed_ts_body, timeout=30
    )
    check(
    ...
    session_response = requests.post(
        BASE_URL + '/v1/sessions', json=session_body, timeout=30
    )
    check(
        'FIXED: POST /v1/sessions returns 201 with a Location header',
        session_response.status_code == 201
        and 'Location' in session_response.headers,
    ...
    if session_response.status_code == 201:
        ...
        delete_response = requests.delete(
            f'{BASE_URL}/v1/sessions/{session_id}', timeout=30
        )
        check(
...
def real_dataset_mixed_dtypes() -> None:
    section(
        'UCI Adult Income: real dataset, mixed dtypes, large int64 ids, timestamps'
    )
    columns = [
        'age',
        'workclass',
    ...
    frame['event_time'] = pd.date_range(
        '2025-01-01', periods=len(frame), freq='min', tz='UTC'
    )
    ...
    accuracy = float(
        (result['prediction'].values == predict_full['income'].values).mean()
    )
    check(
        'accuracy is above trivial baseline (>0.5)',
        accuracy > 0.5,
...
def sessions_lifecycle() -> None:
    ...
    create_response = requests.post(
        BASE_URL + '/v1/sessions', json=body, timeout=30
    )
    check(
    ...
    predict_response = requests.post(
        f'{BASE_URL}/v1/sessions/{session_id}/predictions',
        json=predict_body,
        timeout=30,
    )
    check(
        'session predict returns 200',
...
def error_model() -> None:
    ...
    response = requests.post(
        BASE_URL + '/v1/predictions', json=unsupported_task, timeout=30
    )
    check(
    ...
    check(
        ...
        malformed_response.status_code == 400,
    ...
    check(
        ...
        unknown_session_response.status_code == 404,
...
def admission_control() -> None:
    section(
        'admission control: oversized context should reject cleanly, not crash'
    )
    ...
    body = {
        ...
        'context': {
            'instance_table': {
                ...
                'rows': [
                    [
                        *rng.uniform(-1, 1, n_features).tolist(),
                        'a' if i % 2 == 0 else 'b',
                    ]
    ...
    try:
        response = requests.post(
            BASE_URL + '/v1/predictions', json=body, timeout=120
        )
        check(
        ...
        check(
            'oversized request does not crash the connection', False, str(error)
        )

    ready_response = requests.get(BASE_URL + '/v1/health/ready', timeout=30)
    check(
        'service still ready after oversized request',
...
def concurrency() -> None:
    ...
    def _call(_: int) -> int:
        response = requests.post(
            BASE_URL + '/v1/predictions', json=body, timeout=60
        )
    ...
    ready_response = requests.get(BASE_URL + '/v1/health/ready', timeout=30)
    check(
        'service still ready after concurrency burst',
        ready_response.status_code == 200,
    )
...
def main() -> None:
    if not BASE_URL:
        print('Set KUMO_RELATIONAL_NIM_BASE_URL to run the e2e checks.')
        sys.exit(2)
