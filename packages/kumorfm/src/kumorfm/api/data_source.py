...
from kumorfm.api.typing import WITH_PYDANTIC_V2, compatible_field_validator
...
class StartFileUploadRequest:
    ...
    # Refer: https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html
...
class FileConnectorResourceConfig:
    r"""A configuration of a file-based connector (S3, GCS, or ADLS)."""
...
class SnowflakeConnectorResourceConfig:
    ...
    @compatible_field_validator(
        'name', 'account', 'warehouse', 'database', 'schema_name'
    )
    def strip_inputs(cls, value: str) -> str:
        return value.strip()
...
class DatabricksConnectorResourceConfig:
    ...
    @compatible_field_validator(
        'name', 'host', 'cluster_id', 'warehouse_id', 'catalog'
    )
    def strip_inputs(cls, value: str) -> str:
        return value.strip()
...
class BigQueryCredentials:
    ...
    are obtained from the service account key JSON file generated by the Google
    Cloud SDK.
...
Credentials = (
    UsernamePassword | KeyPair | BigQueryCredentials | DatabricksCredentials
)
