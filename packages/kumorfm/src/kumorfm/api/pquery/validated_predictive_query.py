...
from kumorfm.api.pquery.AST.ast_node import ArrayDtype
from kumorfm.api.pquery.AST.date_offset_range import DateOffsetRange
from kumorfm.api.pquery.AST.location_interval import ASTQueryLocationInterval
from kumorfm.api.pquery.utils import QueryType, maybe_bold
from kumorfm.api.typing import BoolOp, Dtype, ProblemType, RelOp, Stype
...
class ParsedPredictiveQuery:
    ...
    def to_autoregressive_query(self) -> Self:
        r"""Modifies the query for usage as an autoregressive label generator.

        In particular,
        * removes what-if clauses
        * removes any entity filters
        * removes target condition for single, numerical aggregations
        """
        out = copy.copy(self)

        out.whatif_ast = None  # Remove what-if clauses.

        if isinstance(out.entity_ast, Filter):  # Remove entity filters:
