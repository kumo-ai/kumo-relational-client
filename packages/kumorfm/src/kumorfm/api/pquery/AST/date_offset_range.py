...
from kumorfm.api.typing import TimeUnit
...
class DateOffsetRange:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.is_open:
            ...
            self.start_date_offset = self._to_date_offset(self.unit, self.start)
        self.end_date_offset = self._to_date_offset(self.unit, self.end)

    @staticmethod
    def _to_date_offset(unit: TimeUnit, value: int) -> pd.DateOffset:
    ...
    def timedelta_repr(td: pd.Timedelta, unit: TimeUnit) -> str:
        ...
        r_minutes_repr = (
            f'{r_minutes} minutes' if r_minutes > 1 else f'{r_minutes} minute'
        )
    ...
    def date_offset_multiply(
        ...
        date_offset_product = pd.DateOffset(
            **{k: v * multiplier for (k, v) in list(date_offset.kwds.items())}
        )
    ...
    def date_offset_gt(
        ...
        r"""Compare between `date_offset_left` and `date_offset_right`.
        ...
        Returns:
            (bool): Whether `date_offset_left` is larger than
                `date_offset_right`.
        """
        abs_date = pd.Timestamp('1970-01-01')
        assert isinstance(abs_date, pd.Timestamp)
    ...
    @property
    def timeframe(self) -> pd.Timedelta | None:
        ...
        abs_date = pd.Timestamp('1970-01-01')
        assert isinstance(abs_date, pd.Timestamp)
        # Additional type checks to not confuse type checkers
        start_date_offset = self.start_date_offset
        assert isinstance(start_date_offset, pd.DateOffset)
        return (abs_date + self.end_date_offset) - (
            abs_date + start_date_offset
        )
