...
def spaced_graph():
    ...
    orders = pd.DataFrame({
        ...
        'TIME': ['2025-01-0%d' % (i % 8 + 1) for i in range(8)],
    })
...
def test_canonical_fqn_drops_the_quoting(full_name: str,
                                         expected: str) -> None:
...
def test_the_ast_holds_the_name_the_data_uses(spaced_graph) -> None:
    ...
    """
    query = ('PREDICT SUM(ORDERS.`Line Total`, 0, 30, days) '
             'FOR EACH `My People`.`Customer ID`')
    validated = parse_query_locally(query,
...
def test_quoting_a_spellable_name_is_accepted(spaced_graph) -> None:
    ...
    bare = parse_query_locally(
        ...
        'FOR EACH `My People`.`Customer ID`', graph_definition)
