...
def test_changed_arguments_still_rebuild_the_client() -> None:
    ...
    kumorfm.init_databricks_serving('kumo-rfm-2', workspace_client=_Workspace())
...
def test_each_thread_gets_its_own_client() -> None:
    ...
    thread = threading.Thread(
        target=lambda: seen.append(kumorfm.global_state.client)
    )
    thread.start()
    thread.join()
...
def test_serving_reinit_replaces_cached_clients_in_other_threads() -> None:
    ...
    def _worker() -> None:
        try:
            seen.append(kumorfm.global_state.client)
            first_ready.set()
...
def test_url_mode_is_unchanged(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    called: list[bool] = []
    monkeypatch.setattr(
        KumoClient, 'authenticate', lambda self: called.append(True)
    )
