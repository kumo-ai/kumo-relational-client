...
``SDFMClient`` left the engine's pool open. ``close_client`` is the release,
...
def test_close_client_is_gated_like_the_other_entry_points() -> None:
    r"""The engine is reachable only through ``SDFMClient``; a release that
...
def test_the_next_use_rebuilds_rather_than_fails(
    ...
    second = kumorfm.global_state.client
...
def test_a_failing_close_still_evicts(
    ...
    assert kumorfm.global_state.client is not client
