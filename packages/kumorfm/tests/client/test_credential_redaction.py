...
import kumorfm
import pytest
from kumorfm.client.client import KumoClient, redact_url, scrub_userinfo
...
def _restore_global_state():
    before = (kumorfm.global_state._url, kumorfm.global_state._api_key)
    yield
    kumorfm.global_state._url, kumorfm.global_state._api_key = before


def test_global_state_repr_does_not_render_the_api_key() -> None:
    kumorfm.global_state._api_key = _SECRET

    assert _SECRET not in repr(kumorfm.global_state)


def test_global_state_repr_still_names_the_deployment() -> None:
    kumorfm.global_state._url = 'https://nim.example'

    assert 'https://nim.example' in repr(kumorfm.global_state)


def test_api_key_is_still_readable_through_the_attribute() -> None:
    kumorfm.global_state._api_key = _SECRET

    assert kumorfm.global_state._api_key == _SECRET
...
def test_invalid_url_message_does_not_leak_url_userinfo() -> None:
    with pytest.raises(ValueError) as caught:
        KumoClient(f'ftp://svc:{_SECRET}@nim.example')
...
def test_init_log_line_does_not_leak_url_userinfo(
    ...
    with caplog.at_level(logging.INFO, logger='kumorfm'):
        kumorfm.init(url=url, api_key=_SECRET)
