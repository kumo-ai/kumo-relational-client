...
DTYPE_WHITELIST = frozenset({
    'float32', 'float64', 'int32', 'int64', 'bool', 'string', 'stringlist',
    'timestamp[us]',
})
NUMERIC_DTYPES = frozenset({'float32', 'float64', 'int32', 'int64'})
JSON_SAFE_INT = 9007199254740991
RFC3339 = re.compile(r'\d{4}-\d{2}-\d{2}[Tt ]\d{2}:\d{2}:\d{2}(\.\d{1,6})?'
                     r'([Zz]|[+-]\d{2}:\d{2})')
    ...
    if dtype == 'string':
        ...
        for index, item in enumerate(value):
            if not isinstance(item, str):
                problems.append(f'{path}[{index}]: expected str in stringlist, '
        ...
        if isinstance(value, bool):
            ...
            if dtype == 'int64' and abs(int(value)) > JSON_SAFE_INT:
                problems.append(f'{path}: int64 outside the JSON safe range '
                                f'must travel as a base-10 string')
        elif isinstance(value, str):
            if re.fullmatch(r'[+-]?[0-9]+', value) is None:
                problems.append(f'{path}: expected a base-10 integer string, '
                                f'got {value!r}')
        else:
            problems.append(
        ...
        if not isinstance(value, str) or not RFC3339.fullmatch(value):
            problems.append(f'{path}: expected an RFC 3339 string, '
                            f'got {value!r}')
...
def payload_fidelity_problems(payload: dict[str, Any]) -> list[str]:
    ...
    for section in ('context', 'predict'):
        tables = [('instance_table', payload[section]['instance_table'],
    ...
    for index, label in enumerate(target.get('classes', [])):
        if not isinstance(label, str):
            ...
            problems.append(f'task.target.classes[{index}]: {label!r} is not '
...
def _orders() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        'TIME': pd.to_datetime('2024-01-01') + pd.to_timedelta(
...
def _regression_task() -> TaskTable:
    return TaskTable(
        ...
        context_df=pd.DataFrame({
            ...
            'ANCHOR_TIMESTAMP': pd.to_datetime(['2025-02-20'] *
                                               (NUM_USERS - 2)),
        }),
...
def feature_payload(feature: Any) -> tuple[Graph, dict[str, Any]]:
    ...
    graph = Graph.from_data({'USERS': users, 'ORDERS': _orders()},
                            verbose=False)
    payload = KumoRFM(graph, verbose=False).materialize_task(
        _regression_task(), verbose=False)[0].payload
...
FEATURE_CASES = {
    ...
    'datetime_tz': pd.date_range('2024-01-01', periods=NUM_USERS,
                                 tz=_TZ),
    ...
    'bytes': pd.Series([bytes([index % 3, 7]) for index in range(NUM_USERS)],
                       dtype=object),
    'decimal': pd.Series([Decimal(f'{index}.1234')
                          for index in range(NUM_USERS)], dtype=object),
    ...
    'object_int_with_null': pd.Series([1, 2, None] * (NUM_USERS // 3),
                                      dtype=object),
    'object_float': pd.Series([1.5, 2.5, 3.5] * (NUM_USERS // 3),
                              dtype=object),
    'object_bool': pd.Series([True, False] * (NUM_USERS // 2), dtype=object),
    'object_mixed': pd.Series([1, 'a', 2.5, None] * (NUM_USERS // 4),
                              dtype=object),
    'object_all_null': pd.Series([None] * NUM_USERS, dtype=object),
    'object_uuid': pd.Series([uuid.uuid4() for _ in range(NUM_USERS)],
                             dtype=object),
    'object_date': pd.Series([datetime.date(2024, 1, 1 + index % 28)
                              for index in range(NUM_USERS)], dtype=object),
    'object_datetime': pd.Series([datetime.datetime(2024, 1, 1 + index % 28)
                                  for index in range(NUM_USERS)],
                                 dtype=object),
    'object_timedelta': pd.Series([pd.Timedelta(days=index)
                                   for index in range(NUM_USERS)],
                                  dtype=object),
    ...
    'nullable_float64': pd.array([1.5, 2.5, None] * (NUM_USERS // 3),
                                 dtype='Float64'),
    'nullable_boolean': pd.array([True, False, None] * (NUM_USERS // 3),
                                 dtype='boolean'),
    'string_extension': pd.array([f's{index}' for index in range(NUM_USERS)],
                                 dtype='string'),
    ...
    'ndarray_float': pd.Series([np.array([1.0, 2.0])] * NUM_USERS,
                               dtype=object),
...
def test_decimal_column_is_a_number_not_a_string() -> None:
    # Regression: bugs/data-decimal-column-declared-string-but-numerical-500s.md
    # -- `dtype: 'string'` beside `stype: 'numerical'` is what the NIM answers
    # with an HTTP 500 the SDK then reports as GPU capacity pressure.
    ...
    assert feature_schema(payload) == {'dtype': 'float64',
                                       'stype': 'numerical'}
...
def test_decimal_column_is_not_inferred_as_free_text() -> None:
    # Regression: bugs/data-decimal-column-inferred-as-text-and-split-on-the-
    # point.md -- a high-cardinality Decimal column became `Stype.text`, whose
    # normalizer strips the decimal point, so `18.1234` travelled as
    # `['18', '1234']`.
    graph, payload = feature_payload(
        [Decimal(f'{index}.1234') for index in range(NUM_USERS)])
...
def test_object_and_category_columns_send_what_they_declare(feature) -> None:
    # Regression: bugs/data-object-and-category-columns-declare-string-but-send-
    # numbers.md -- these declared `string` and sent JSON numbers, and the NIM
...
def test_numeric_list_columns_travel_as_lists_of_strings(feature) -> None:
    # Regression: bugs/rfm-int-float-list-columns-declared-stringlist.md --
    # `stringlist` was declared for every list column while the items went out
    # as JSON numbers, which the NIM's cell union has no branch for.
    _, payload = feature_payload(feature)
...
def test_timedelta_column_is_serializable_as_the_quantity_it_is() -> None:
    # Regression: bugs/rfm-timedelta-bytes-columns-unserializable.md -- the
    # graph typed the column `timedelta`/`numerical`, `validate()` passed, and
    # the first `predict()` died inside `json.dumps` naming neither table nor
    # column.
...
def test_bytes_column_is_serializable_as_the_string_it_declares() -> None:
    # Regression: bugs/rfm-timedelta-bytes-columns-unserializable.md
    _, payload = feature_payload(
        pd.Series([bytes([index % 3, 7]) for index in range(NUM_USERS)],
                  dtype=object))
...
def test_boolean_multiclass_target_labels_match_its_declared_dtype() -> None:
    # Regression: bugs/rfm-bool-target-imputation-rejected.md -- the target was
    # declared `dtype: 'bool'` while its classes went out as `str(np.True_)`,
    # which the NIM refuses; and a boolean *multiclass* label cannot be mapped
    # back to a declared class at all, so the column travels as its two labels.
    users = pd.DataFrame({
...
def test_tz_aware_anchor_timestamps_are_converted_not_refused() -> None:
    # Regression: bugs/rfm-tz-aware-anchor-timestamps-rejected.md -- `predict`
    # returns `ANCHOR_TIMESTAMP` tz-aware and `predict_task`'s docstring points
    # callers back at it, but `astype('datetime64[ns]')` raises on a tz-aware
    # series rather than converting it.
    users = pd.DataFrame({'USER_ID': np.arange(NUM_USERS, dtype='int64')})
    graph = Graph.from_data({'USERS': users, 'ORDERS': _orders()},
                            verbose=False)
    task = TaskTable(
...
def test_tz_aware_scalar_anchor_time_is_accepted() -> None:
    # Regression: bugs/rfm-tz-aware-anchor-timestamps-rejected.md -- the scalar
    # path failed separately, comparing a tz-aware Timestamp against the
    # graph's tz-naive range.
    users = pd.DataFrame({'USER_ID': np.arange(NUM_USERS, dtype='int64')})
    graph = Graph.from_data({'USERS': users, 'ORDERS': _orders()},
                            verbose=False)
    model = KumoRFM(graph, verbose=False)
