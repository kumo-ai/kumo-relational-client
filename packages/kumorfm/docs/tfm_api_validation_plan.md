...
`scripts/generate_tfm_api.py`, the generated `kumorfm/client/generated/tfm_api.py`
...
- `kumorfm/client/rfm.py` row mapping vs parsed response fields
- `kumorfm/rfm/payload.py` request envelope vs `PredictionRequest`
...
Add request-side coverage for `kumorfm/rfm/payload.py`, which is handwritten and
