...
`KumoRFM.materialize_task()` generates the Universal TFM requests that
...
therefore requires the compiled `kumorfm.kumolib` extension. Install the SDK
...
python -c "import kumorfm.kumolib"
```
...
`KumoRFM.sanitization_report` exposes backend-neutral diagnostics. Local
