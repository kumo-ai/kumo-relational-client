...
def format_invalid_params(params: Sequence[Any] | None) -> str:
    ...
    Note:
        ...
        ``nemotron_predict``, and the package both do depend on is a SQL-connector
...
class UnknownModelError(PredictError):
    r"""A request named a model no registered adapter serves.

    Raised by :meth:`~nemotron_predict.PredictClient.capabilities` and when dispatching
...
class MissingExtraError(PredictError):
    ...
    def __init__(self, extra: str, package: str) -> None:
        super().__init__(
            ...
            f'`pip install nemotron-predict-client[{extra}]`',
            code='MISSING_EXTRA',
            details={'extra': extra, 'package': package},
        )


class NimRequestError(PredictError):
    r"""The NIM answered, and the answer was an error.

    Distinguished from :class:`PredictError` by carrying the HTTP
