...
:class:`~nemotron_predict.core.transport.Transport` speaks HTTP to a NIM at a base
...
one of them refuses: each raises an :class:`PredictError` naming the endpoint and
...
class ServingTarget:
    ...
    def _require_open(self) -> None:
        r"""Part of the transport contract ``PredictClient._predict`` relies on.
        ...
        if self._closed:
            raise PredictError(
                'this client is closed; construct a new PredictClient',
                code='INVALID_CONFIGURATION',
            )
