...
class ModelCapabilities:
    r"""What a model supports, discoverable via ``PredictClient.capabilities(model)``.
    ...
    - ``model``: The model id, e.g. ``'nemotron-relational-v1'``.
    - ``request_type``: The name of the adapter's request class, for
      ...
      handles and are not importable from ``nemotron_predict``.
...
class ModelAdapter(ABC):
    r"""The extension point for serving another model through ``PredictClient``.
    ...
    is added to a client with ``PredictClient._register``.
    """
    ...
    def predict(
        ...
        ``ServingTarget`` raises an ``PredictError`` naming the reason from
        ...
        :class:`~nemotron_predict.adapters.relational.NemotronRelationalAdapter` does.
    ...
    def close(self) -> None:  # noqa: B027 - optional hook, see below
        ...
        Called by :meth:`~nemotron_predict.PredictClient.close` for every registered
