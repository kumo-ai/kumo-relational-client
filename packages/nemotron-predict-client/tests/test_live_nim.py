...
def test_nim_reports_ready(client: PredictClient):
    assert client.health_ready() is True


def test_tabicl_predict_returns_expected_shape(client: PredictClient):
...
def test_tabicl_predict_regression_returns_quantiles(client: PredictClient):
    context = pd.DataFrame(
        {
