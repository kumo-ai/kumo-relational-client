...
def test_predict_unknown_model_raises():
    ...
    with pytest.raises(PredictError):
        PredictClient(url=_URL)._predict(_UnknownRequest())
...
def test_client_forwards_timeout_and_max_retries():
    client = PredictClient(_URL, timeout=12.0, max_retries=7)
...
# while PredictClient._predict had started calling it. Nothing conflicted
