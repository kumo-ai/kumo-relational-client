...
def test_nemotron_relational_shim_does_not_expose_driver():
    from nemotron_predict import relational as nemotron_relational_shim
...
def test_nemotron_relational_shim_namespace_is_pinned():
    ...
    # This namespace is the documented NemotronRelational surface (README, quickstart
...
def test_nemotron_relational_shim_withholds_names_the_supported_api_cannot_reach():
    ...
    Neither is reachable through `PredictClient`: the first is returned only by
...
def test_capabilities_advertise_supported_task_types():
    from nemotron_predict.adapters.relational import RFM_TASK_TYPES
...
def test_concurrent_clients_predict_against_their_own_endpoint(
    ...
    clients = {
        'A': PredictClient(url='https://tenant-a.example', api_key='key-A'),
        'B': PredictClient(url='https://tenant-b.example', api_key='key-B'),
    }
