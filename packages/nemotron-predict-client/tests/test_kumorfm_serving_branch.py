...
def test_a_url_client_still_reprs_with_its_url() -> None:
    ...
    assert 'https://nim.example.com:8000' in repr(
        PredictClient('https://nim.example.com:8000')
    )
...
def test_the_two_endpoint_validation_layers_agree(endpoint: str) -> None:
    ...
    with pytest.raises(PredictError) as ours:
        PredictClient.for_databricks_serving(endpoint)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            _missing_sdk,
            'MISSING_EXTRA',
            'pip install nemotron-predict-client[databricks-serving]',
...
def test_engine_init_failures_are_translated_at_the_boundary(
    ...
    who installed nemotron-predict-client[databricks-serving] to install a package they
