...
from nemotron_relational import in_tmux
from nemotron_relational.api.source_table import UnavailableSourceTable
from nemotron_relational.api.table import Column as ColumnDefinition
...
from nemotron_relational.rfm.base.utils import to_datetime
from nemotron_relational.rfm.infer import (
    infer_dtype,
    infer_primary_key,
    infer_stype,
    infer_time_column,
)
from nemotron_relational.runmode import MissingType
...
class Table(ABC):
    ...
    def remove_column(self, name: str) -> Self:
        ...
        A :class:`~nemotron_relational.rfm.Table` does not know which graph holds it, so
        ...
        Call :meth:`~nemotron_relational.rfm.Graph.unlink` first when removing a linked
        foreign key; otherwise :meth:`~nemotron_relational.rfm.Graph.validate` rejects the
        graph.
    ...
    def _sanitize(
        df: pd.DataFrame,
        dtype_dict: Mapping[str, Dtype | None] | None = None,
        stype_dict: Mapping[str, Stype | None] | None = None,
    ) -> pd.DataFrame:
