...
from nemotron_relational.rfm.backend.snow.binding import paramstyle
...
class SnowSampler(SQLSampler):
    ...
    def _by_pkey(
        ...
        sql = 'WITH TMP as (\n  SELECT\n    f.index as __NEMOTRON_BATCH__,\n'
        if self.table_dtype_dict[table_name][key].is_int():
            sql += '    f.value::NUMBER as __NEMOTRON_ID__\n'
        elif self.table_dtype_dict[table_name][key].is_float():
            sql += '    f.value::FLOAT as __NEMOTRON_ID__\n'
        else:
            sql += '    f.value::VARCHAR as __NEMOTRON_ID__\n'
        sql += (
        ...
        tmp = table.append_column(
            '__NEMOTRON_ID__', pa.array(range(len(table)))
        )
        gb = tmp.group_by('__NEMOTRON_BATCH__').aggregate(
            [('__NEMOTRON_ID__', 'min')]
        )
        table = table.take(gb['__NEMOTRON_ID___min'])
    ...
    def _by_fkey(
        ...
        if self.table_dtype_dict[table_name][foreign_key].is_int():
            sql += '    f.value[0]::NUMBER as __NEMOTRON_ID__'
        elif self.table_dtype_dict[table_name][foreign_key].is_float():
            sql += '    f.value[0]::FLOAT as __NEMOTRON_ID__'
        else:
            sql += '    f.value[0]::VARCHAR as __NEMOTRON_ID__'
        if end_time is not None and start_time is not None:
            sql += (
                ',\n'
                '    f.value[1]::TIMESTAMP_NTZ as __NEMOTRON_END_TIME__,\n'
                '    f.value[2]::TIMESTAMP_NTZ as __NEMOTRON_START_TIME__'
            )
        sql += (
            f'\n'
            f'  FROM TABLE(FLATTEN(INPUT => PARSE_JSON(?))) f\n'
    ...
    def _by_time(
        ...
        if self.table_dtype_dict[table_name][foreign_key].is_int():
            ...
            sql += '    f.value[0]::VARCHAR as __NEMOTRON_ID__,\n'
        sql += '    f.value[1]::TIMESTAMP_NTZ as __NEMOTRON_END_TIME__'
        if min_offset is not None:
            sql += ',\n    f.value[2]::TIMESTAMP_NTZ as __NEMOTRON_START_TIME__'
        sql += (
            f'\n'
