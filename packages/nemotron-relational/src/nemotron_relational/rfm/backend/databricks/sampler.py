...
from nemotron_relational.rfm.backend.databricks import DatabricksTable
from nemotron_relational.rfm.backend.databricks.table import BACKTICK
...
class DatabricksSampler(SQLSampler):
    ...
    def _flatten(self, sql: str, rows: list) -> pa.Table:
        ...
        single batch space via the ``__NEMOTRON_BATCH__`` column.
        """
        ...
        for chunk in chunks:
            ...
            batch = table['__NEMOTRON_BATCH__'].cast(pa.int64())
            batch = batch.to_numpy(zero_copy_only=False) + offset
            index = table.schema.get_field_index('__NEMOTRON_BATCH__')
            table = table.set_column(
                index, '__NEMOTRON_BATCH__', pa.array(batch)
            )
            tables.append(table)
            offset += len(chunk)
    ...
    def _sample_entity_table(
        ...
        if entity_ids is not None:
            ...
            join = (
                ...
                f'(__pos__, __NEMOTRON_ID__)\n'
                f'  ON {key_ref} = __ids__.__NEMOTRON_ID__'
            )
            parameters = [
    ...
    def _by_pkey(
        ...
        sql = (
            f'WITH TMP AS (\n'
            f'  SELECT pos AS __NEMOTRON_BATCH__, col AS __NEMOTRON_ID__\n'
            f"  FROM posexplode(from_json(?, 'array<{self._elem_type(dtype)}"
            f">'))\n"
            f')\n'
            f'SELECT '
            f'TMP.__NEMOTRON_BATCH__ as __NEMOTRON_BATCH__, '
            f'{", ".join(projections)}\n'
            f'FROM TMP\n'
        ...
        tmp = table.append_column(
            '__NEMOTRON_ROW__', pa.array(range(len(table)))
        )
        gb = tmp.group_by('__NEMOTRON_BATCH__').aggregate(
            [('__NEMOTRON_ROW__', 'min')]
        )
        table = table.take(gb['__NEMOTRON_ROW___min'])

        batch = table['__NEMOTRON_BATCH__'].cast(pa.int64()).to_numpy()
    ...
    def _by_fkey(
        ...
        select = ['pos AS __NEMOTRON_BATCH__']
        if end_time is not None and start_time is not None:
            select.append('col.id AS __NEMOTRON_ID__')
            select.append('col.e AS __NEMOTRON_END_TIME__')
            select.append('col.s AS __NEMOTRON_START_TIME__')
        else:
            select.append('col AS __NEMOTRON_ID__')
        sql = (
            f'WITH TMP AS (\n'
        ...
        if end_time is not None and start_time is not None:
            ...
            sql += (
                f' AND {time_ref} <= TMP.__NEMOTRON_END_TIME__\n'
                f' AND {time_ref} > TMP.__NEMOTRON_START_TIME__\n'
    ...
    def _by_time(
        ...
        select = [
            'pos AS __NEMOTRON_BATCH__',
            'col.id AS __NEMOTRON_ID__',
            'col.e AS __NEMOTRON_END_TIME__',
        ]
        if min_offset is not None:
            select.append('col.s AS __NEMOTRON_START_TIME__')
        sql = (
        ...
        if start_time is not None:
            sql += f'AND {time_ref} > TMP.__NEMOTRON_START_TIME__\n'
