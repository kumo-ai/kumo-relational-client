...
_SIZE_LIMIT_MSG = (
    ...
    "'https://github.com/NVIDIA/nemotron-predict-client' if you must go "
...
def _no_session_reason(random_seed: int | None) -> str:
    ...
    """
    cause = (
        'NEMOTRON_PREDICT_DISABLE_SESSIONS is set'
...
def _nim_failure_error(
    ...
    if isinstance(error, Timeout):
        return NimFailureError(
            ...
            'timeout. Raise it with PredictClient(url, timeout=...), or retry '
    ...
    return NimFailureError(
        ...
        f"'https://github.com/NVIDIA/nemotron-predict-client'. "
...
class NemotronRelational:
    r"""Run NemotronRelational predictions over a relational graph.
    ...
    def batch_mode(
        ...
        Note:
            ...
            ``NEMOTRON_PREDICT_DISABLE_SESSIONS`` rules that out -- in which case every
    ...
    def _get_context(
        ...
        if len(num_neighbors) > _MAX_HOPS:
            raise ValueError(
                ...
                f"'https://github.com/NVIDIA/nemotron-predict-client' if you "
