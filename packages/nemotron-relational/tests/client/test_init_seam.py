...
def test_repeated_init_does_not_rebuild_the_client() -> None:
    ...
    nemotron_relational.init_databricks_serving(
        'nemotron-relational-v1', workspace_client=workspace
    )
...
def test_each_thread_gets_its_own_client() -> None:
    """The cache is per thread, so an existing NemotronRelational keeps the client it was
    built with.
    """
    nemotron_relational.init_databricks_serving(
        'nemotron-relational-v1', workspace_client=_Workspace()
...
def test_url_mode_is_unchanged(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    nemotron_relational.global_state.clear()
    monkeypatch.delenv('NEMOTRON_PREDICT_API_ENDPOINT', raising=False)
