...
"""``nemotron_relational.rfm.init_databricks_serving``, without the native extension.

``nemotron_relational/rfm/backend/local/__init__.py`` does a bare ``import
nemotron_relational.relationallib`` at module scope and turns any failure into a RuntimeError, so
``import nemotron_relational.rfm`` is impossible wherever the compiled extension is absent.
...
def stubbed_relationallib(monkeypatch: pytest.MonkeyPatch) -> Any:
    ...
    if 'nemotron_relational.relationallib' in sys.modules:
        pytest.skip(
            'the real relationallib is present; stubbing it would mutate the real '
            'module and the recording would not apply'
        )
    stub = _RecordingModule('nemotron_relational.relationallib')
    monkeypatch.setitem(sys.modules, 'nemotron_relational.relationallib', stub)
    return stub


@pytest.fixture()
def rfm_engine(monkeypatch: pytest.MonkeyPatch) -> Any:
    """Import nemotron_relational.rfm with the local-sampler guard satisfied."""
    if 'nemotron_relational.relationallib' not in sys.modules:
        monkeypatch.setitem(
            sys.modules,
            'nemotron_relational.relationallib',
            types.ModuleType('nemotron_relational.relationallib'),
        )
    try:
        import nemotron_relational.rfm as engine
    except RuntimeError as error:  # pragma: no cover - environment dependent
        pytest.skip(
            f'nemotron_relational.rfm unusable even with the guard stubbed: {error}'
        )
    yield engine
...
def test_the_serving_path_never_needs_the_native_sampler(
    ...
    Scope, stated plainly: this observes access *after* ``nemotron_relational.rfm`` is
    already imported. Import-time use -- the bare ``import nemotron_relational.relationallib`` in
