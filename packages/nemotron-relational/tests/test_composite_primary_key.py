...
from nemotron_relational.rfm.base.composite_key import (
    SEPARATOR,
    decode_value,
    encode_frame,
    encode_values,
...
def test_a_warehouse_that_folds_differently_is_refused(superstore) -> None:
    ...
    """
    from nemotron_relational.rfm.base import composite_key as module
...
def test_tuple_seeds_are_folded_into_identities() -> None:
    from nemotron_relational.rfm.rfm import _encode_composite_indices
...
def test_a_prediction_is_reported_against_the_declared_columns() -> None:
    from nemotron_relational.rfm.rfm import _decode_composite_entities
...
def test_declaring_the_same_identity_twice_reuses_one_column() -> None:
    ...
    derived = [
        name
        for name in table._data.columns
        if name.startswith('__nemotron_key_')
    ]
...
def test_a_rejected_link_leaves_no_column_behind(superstore) -> None:
    ...
    stray = [
        ...
        if column.name.startswith('__nemotron_key_')
    ]
    assert stray == []
...
def test_an_identity_too_long_to_name_is_refused() -> None:
    from nemotron_relational.rfm.base.composite_key import encode_identity
