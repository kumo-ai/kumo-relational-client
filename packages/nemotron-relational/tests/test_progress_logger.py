...
def test_taskbar_escapes_only_reach_a_verbose_tty(
    ...
    monkeypatch.setattr(nemotron_relational, 'in_notebook', lambda: False)
...
def test_in_terminal_survives_a_stdout_without_isatty(monkeypatch) -> None:
    from nemotron_relational.utils.progress_logger import _in_terminal
...
def test_in_terminal_is_false_in_a_notebook(monkeypatch) -> None:
    ...
    monkeypatch.setattr(nemotron_relational, 'in_notebook', lambda: True)
