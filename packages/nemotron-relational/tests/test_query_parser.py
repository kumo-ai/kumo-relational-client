...
from nemotron_relational.api.typing import ProblemType
...
def test_kumo_rfm_parse_query_delegates_strings_to_local_parser(
    ...
    with patch(
        'nemotron_relational.rfm.rfm.parse_query_locally',
        return_value=ltv,
    ) as mock_parse:
...
def test_demo_prefix_offsets_assuming_location(
    ...
    from nemotron_relational.pql.validator.rfm_validator import RfmValidator
...
def test_parse_valid_queries_against_rfm_graph(
    ...
    parsed = NemotronRelational(user_store_graph, verbose=False)._parse_query(
        query
    )
...
def test_parse_string_entity_ids(
    ...
    parsed = NemotronRelational(string_user_graph, verbose=False)._parse_query(
        query
    )
...
def test_parse_invalid_queries_report_useful_errors(
    ...
    with pytest.raises(ValueError) as exc_info:
        NemotronRelational(user_store_graph, verbose=False)._parse_query(query)
...
class _RecordCollector(logging.Handler):
    r"""Collects records straight off the ``nemotron_relational`` logger.
...
@contextmanager
def _capture_nemotron_relational_logs() -> Iterator[_RecordCollector]:
...
def _restored_logging() -> Iterator[None]:
    ...
    names = ['nemotron_relational', 'matplotlib', 'urllib3', 'snowflake']
...
def test_parse_failure_does_not_log_the_raw_query_at_warning(
    ...
    with _capture_nemotron_relational_logs() as captured:
        with pytest.raises(ValueError, match=secret):
            NemotronRelational(user_store_graph, verbose=False)._parse_query(
                query
            )
...
def test_importing_and_using_the_driver_leaves_the_root_logger_alone() -> None:
    ...
    import nemotron_relational._logging

    with _restored_logging():
        root = logging.getLogger()
        before = list(root.handlers)
        nemotron_relational._logging.initialize_logging()
        assert list(root.handlers) == before

        logger = logging.getLogger('nemotron_relational')
        assert logger.handlers, (
            'nemotron_relational must handle its own records'
        )
...
def test_a_valid_query_still_parses(user_store_graph: Graph) -> None:
    ...
    with redirect_stdout(stdout):
        parsed = NemotronRelational(
            user_store_graph, verbose=False
        )._parse_query(
