...
from nemotron_relational.api.pquery.AST import Column, Condition, Constant
from nemotron_relational.api.rfm import RFMPredictResponse
from nemotron_relational.api.rfm.context import REV_REL, EdgeLayout
...
def test_temporal_rfm(
    ...
    assert str(model) == 'NemotronRelational()'
...
def test_get_train_table(
    ...
    model = NemotronRelational(string_user_graph, verbose=False)
...
def test_optimize_left_unset_does_not_warn(user_store_graph: Graph) -> None:
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        NemotronRelational(user_store_graph, verbose=False)
...
def test_lag_timesteps(
    ...
    for i in range(1, 5):
        ...
        pd.testing.assert_series_equal(
            df['TARGET'],
            pred_df[f'__nemotron_arl{i - 1}__'],
            check_names=False,
        )
...
def test_explanation_warning_display():
    ...
    with (
        patch('nemotron_relational.rfm.rfm.in_notebook', return_value=True),
        patch('nemotron_relational.rfm.rfm.display') as mock_display,
    ):
        with_warning.print()
...
def test_over_cap_batch_size_suggests_a_value_under_the_cap() -> None:
    ...
    """
    from nemotron_relational.rfm.rfm import _MAX_PRED_SIZE
