...
from kumo_relational_client.core.serving import ServingTarget
...
def _translate_init_error(error: BaseException) -> RelationalError:
    ...
    ``except RelationalError`` could catch a wrong API key or an unreachable NIM.
    ...
    try:
        ...
        return RelationalError(str(error), code='TRANSPORT_ERROR')
    if isinstance(error, AuthenticationError):
        return RelationalError(str(error), code='AUTHENTICATION_FAILED')
    if isinstance(error, NimTimeoutError | NimUnreachableError):
        return RelationalError(str(error), code='TRANSPORT_ERROR')
    return RelationalError(str(error), code='INVALID_CONFIGURATION')
...
def _translate_engine_error(error: Exception, url: str) -> RelationalError:
    ...
    if invalid_response_type is not None and isinstance(
        ...
        return RelationalError(str(error), code='INVALID_RESPONSE')
    if isinstance(error, KeyError):
        message = str(error.args[0]) if error.args else str(error)
        return RelationalError(message, code='INVALID_REQUEST')
    if isinstance(error, (ValueError, TypeError, LookupError)):
        return RelationalError(str(error), code='INVALID_REQUEST')
    return RelationalError(
...
def _validate_task_request(request: KumoRelationalTaskRequest) -> None:
...
def _build_task_table(engine: Any, request: KumoRelationalTaskRequest) -> Any:
...
class KumoRelationalAdapter(ModelAdapter):
    ...
    def predict(
        ...
        request: KumoRelationalRequest | KumoRelationalTaskRequest,
    ) -> pd.DataFrame | Explanation:
        engine = _load_engine()
        ...
        if isinstance(request, KumoRelationalTaskRequest):
