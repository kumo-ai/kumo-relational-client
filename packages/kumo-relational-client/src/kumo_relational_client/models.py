...
if TYPE_CHECKING:
    from kumo_relational_client.base import PredictResult
...
def require_frame(value: Any, name: str) -> pd.DataFrame:
    ...
    adapter, which ``except RelationalError`` does not catch and which names neither
...
class RelationalModel:
        ...
        **engine_options: Any,
        ...
        Args:
            ...
            num_retries: Application-level retries of a failed prediction
                ...
                Distinct from
                ``RelationalClient(max_retries=...)``, which retries at the
                transport.
    ...
    def predict_task(
        ...
        **engine_options: Any,
        ...
        options.update(engine_options)
        request = KumoRelationalTaskRequest(
            graph=self._graph,
...
class TabularModel:
    ...
    building a :class:`KumoTabularRequest` under the owning client.
    ...
    def __init__(
        ...
        self._task = task
        self._target = target
        self._session = KumoTabularSession()
    ...
    def __del__(self) -> None:
        ...
        if session is None or session.id is None:
            return
        try:
            from kumo_relational_client.adapters.tabular import (
