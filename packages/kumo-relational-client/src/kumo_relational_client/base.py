...
class ModelCapabilities:
    r"""What a model supports, discoverable via ``RelationalClient.capabilities(model)``.
    ...
    - ``request_type``: The name of the adapter's request class, for
      ...
      handles and are not importable from ``kumo_relational_client``.
...
class ModelAdapter(ABC):
    r"""The extension point for serving another model through ``RelationalClient``.
    ...
    is added to a client with ``RelationalClient._register``.
    """
    ...
    def predict(
        ...
        ``ServingTarget`` raises an ``RelationalError`` naming the reason from
    ...
    def close(self) -> None:  # noqa: B027 - optional hook, see below
        ...
        Called by :meth:`~kumo_relational_client.RelationalClient.close` for every registered
