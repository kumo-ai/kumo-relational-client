...
def test_rfm_handle_end_to_end_through_client():
    ...
    adapter = _CapturingAdapter(
        'kumo-relational', KumoRelationalRequest, result
    )
    ...
    assert isinstance(req, KumoRelationalRequest)
...
def test_rfm_task_handle_names_allowed_run_modes():
    ...
    with pytest.raises(
        RelationalError, match=r"run_mode must be one of.*'best'"
    ):
...
def test_rfm_handle_rejects_non_boolean_options(option, value):
    ...
    with pytest.raises(
        RelationalError, match=rf'{option} must be a bool'
    ) as exc:
...
def test_rfm_task_handle_rejects_non_boolean_options(option):
    ...
    with pytest.raises(RelationalError, match=rf'{option} must be a bool'):
        client.relational('g').predict_task(
            pd.DataFrame({'ENTITY': [1], 'TARGET': [2]}),
            pd.DataFrame({'ENTITY': [1]}),
            task_type='regression',
...
def test_rfm_handle_rejects_malformed_integer_options(
    ...
    with pytest.raises(
        RelationalError, match=rf'{option} must be an {expected}'
    ) as exc:
        client.relational('g').predict('PREDICT x', **{option: value})
...
def test_rfm_task_handle_rejects_malformed_num_hops():
    ...
    with pytest.raises(
        RelationalError, match=r'num_hops.*integer between 1 and 6'
    ):
        client.relational('g').predict_task(
            pd.DataFrame({'ENTITY': [1], 'TARGET': [2]}),
            pd.DataFrame({'ENTITY': [1]}),
...
def test_rfm_task_handle_rejects_malformed_num_neighbors():
    ...
    with pytest.raises(
        RelationalError, match=r'num_neighbors.*non-negative integers'
    ):
        client.relational('g').predict_task(
            pd.DataFrame({'ENTITY': [1], 'TARGET': [2]}),
            pd.DataFrame({'ENTITY': [1]}),
...
def test_rfm_handle_predict_task_end_to_end_through_client():
    ...
    adapter = _CapturingAdapter(
        'kumo-relational', KumoRelationalTaskRequest, result
    )
    client = _client_with(adapter)
    ...
    assert out is result
    req = adapter.captured
    assert isinstance(req, KumoRelationalTaskRequest)
    assert req.model == 'kumo-relational'
    assert req.graph == 'my-graph'
