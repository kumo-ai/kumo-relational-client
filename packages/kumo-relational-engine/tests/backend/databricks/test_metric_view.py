...
from kumo_relational_engine.graph import Edge
from kumo_relational_engine.rfm import Graph

try:
    import pyarrow as pa
    from kumo_relational_engine.rfm.backend.databricks import Connection
...
def test_from_databricks_metric_view_tracks_internal_connection(
    ...
    connection = _FakeConnection()
    monkeypatch.setattr(
        'kumo_relational_engine.rfm.backend.databricks.connect',
...
def metric_view_graph(connection: 'Connection') -> Graph:
    ...
    from kumo_relational_engine.rfm import ViewConversionWarning
...
def test_from_databricks_wraps_a_driver_exception() -> None:
    ...
    assert isinstance(caught.value, KumoRelationalError)
