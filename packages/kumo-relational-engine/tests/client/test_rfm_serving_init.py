...
"""``kumo_relational_engine.rfm.init_databricks_serving``, without the native extension.

``kumo_relational_engine/rfm/backend/local/__init__.py`` does a bare ``import
kumo_relational_engine.relationallib`` at module scope and turns any failure into a RuntimeError, so
``import kumo_relational_engine.rfm`` is impossible wherever the compiled extension is absent.
...
@pytest.fixture()
def stubbed_relationallib(monkeypatch: pytest.MonkeyPatch) -> Any:
    """Only meaningful when the real extension is absent."""
    if 'kumo_relational_engine.relationallib' in sys.modules:
        pytest.skip(
            'the real relationallib is present; stubbing it would mutate the real '
            'module and the recording would not apply'
        )
    stub = _RecordingModule('kumo_relational_engine.relationallib')
    monkeypatch.setitem(
        sys.modules, 'kumo_relational_engine.relationallib', stub
    )
    return stub
...
def rfm_engine(monkeypatch: pytest.MonkeyPatch) -> Any:
    """Import kumo_relational_engine.rfm with the local-sampler guard satisfied."""
    if 'kumo_relational_engine.relationallib' not in sys.modules:
        monkeypatch.setitem(
            sys.modules,
            'kumo_relational_engine.relationallib',
            types.ModuleType('kumo_relational_engine.relationallib'),
        )
    try:
        import kumo_relational_engine.rfm as engine
    except RuntimeError as error:  # pragma: no cover - environment dependent
        pytest.skip(
            f'kumo_relational_engine.rfm unusable even with the guard stubbed: {error}'
        )
    yield engine
...
def test_the_serving_path_never_needs_the_native_sampler(
    stubbed_relationallib: Any, rfm_engine: Any
) -> None:
    """Nothing on the serving path calls into relationallib.

    Scope, stated plainly: this observes access *after* ``kumo_relational_engine.rfm`` is
    already imported. Import-time use -- the bare ``import kumo_relational_engine.relationallib`` in
    ...
    ``relationallib.NeighborSampler`` in ``backend/local/sampler.py``, and the
    serving path does not sample locally.
    """
    before = list(stubbed_relationallib._accessed)
    ...
    during = [
        n
        for n in stubbed_relationallib._accessed[len(before) :]
        if n != '_accessed'
    ]
    assert during == [], (
        f'the serving path reached into relationallib: {during}'
    )
