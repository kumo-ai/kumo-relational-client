...
def test_init_forwards_the_timeout_to_the_request_client():
    try:
        with _serve() as url:
            ...
            assert kumo_relational_engine.global_state.client._timeout == 7.0
    finally:
...
def test_max_retries_reaches_the_transport_policy():
    """/
    client-max-retries-never-reaches-the-kumo_relational_engine-path.md
...
def test_init_forwards_max_retries_to_the_request_client():
    try:
        with _serve() as url:
            ...
            assert kumo_relational_engine.global_state.client._max_retries == 5
    finally:
        rfm_engine.global_state.reset()
        kumo_relational_engine.global_state.clear()
