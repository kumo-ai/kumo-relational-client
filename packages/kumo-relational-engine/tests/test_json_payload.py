...
from kumo_relational_engine.api.rfm.context import Table
from kumo_relational_engine.api.task import TaskType
from kumo_relational_engine.api.typing import Stype
from kumo_relational_engine.client import NimClient
from kumo_relational_engine.client.generated import tfm_api as tfm_api_module
from kumo_relational_engine.client.rfm import RFMAPI
from kumo_relational_engine.rfm import Graph, KumoRelational, TaskTable
...
def test_payload_encodes_unsafe_int64_as_base10_strings() -> None:
    ...
    payload = (
        KumoRelational(graph, verbose=False)
        .materialize_task(
            task,
            verbose=False,
        )[0]
...
def test_explain_generates_nl_summary_for_v2_1_payload(
    ...
    monkeypatch.setattr(
        'kumo_relational_engine.rfm.rfm.generate_summary', fake_generate
    )
    mock_api.post(
        f'{MOCK_URL}/v1/predictions',
        json=_correlated_response(
