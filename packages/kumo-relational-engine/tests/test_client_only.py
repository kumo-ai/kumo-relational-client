...
Direct `rfm.init(...)` + `KumoRelational(graph).predict(...)` against a NIM used to
...
requires_engine = pytest.mark.skipif(
    rfm_engine is None,
    reason=f'kumo_relational_engine.rfm is not usable in this environment: {_UNUSABLE}',
)
...
def test_authorized_init_passes_the_guard(monkeypatch):
    calls = {}
    monkeypatch.setattr(
        kumo_relational_engine, 'init', lambda **kwargs: calls.update(kwargs)
    )
    monkeypatch.setattr(
        type(kumo_relational_engine.global_state),
        '_url',
        'http://x',
        raising=False,
    )
