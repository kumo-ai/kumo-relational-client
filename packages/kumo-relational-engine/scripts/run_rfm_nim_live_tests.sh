...
usage() {
  cat <<'USAGE'
Run URL-driven KumoRelational NIM live validation.
...
if [[ -n "${RFM_NIM_PYTHON:-}" ]]; then
  ...
  "$PYTHON" -c 'import pytest, requests, kumo_relational_engine' >/dev/null || {
  ...
  if [[ ! -x "$PYTHON" || "$installed_fingerprint" != "$fingerprint" ]]; then
    needs_bootstrap=1
  elif ! "$PYTHON" -c 'import pytest, requests, kumo_relational_engine' >/dev/null 2>&1; then
    needs_bootstrap=1
  fi
