...
_HANDLER_ATTR = '_kumo_relational_engine_handler'


def _install_handler(logger: logging.Logger) -> None:
    r"""Attaches our formatter to the ``kumo_relational_engine`` logger, and only to it.
...
def initialize_logging() -> None:
    ...
    Touches the ``kumo_relational_engine`` logger and nothing else. This used to raise the
    ...
    """
    logger: logging.Logger = logging.getLogger('kumo_relational_engine')
