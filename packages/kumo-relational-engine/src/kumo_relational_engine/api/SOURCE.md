# kumo_relational_engine.api: provenance
...
- All internal `kumoapi.*` imports rewritten to `kumo_relational_engine.api.*`.
...
- Kumo Relational Enterprise platform modules dropped as unreachable from the RFM path:
  `jobs.py`, `online_serving.py`, `distilled_model_plan.py`,
  `data_snapshot.py`, `rbac.py`. Nothing in `kumo_relational_engine`, `kumo_relational_client` or
  `kumo_connectors` imports them, and they describe batch jobs, serving
...
`model_plan.py`. They live in `kumo_relational_engine/runmode.py`, outside this tree, because
