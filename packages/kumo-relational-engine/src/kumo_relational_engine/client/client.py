...
# Mirrors ``kumo_relational_client.core.transport``: well above any real prediction
...
class _Session(requests.Session):
    r"""Mirrors ``kumo_relational_client.core.transport._Session``.
    ...
    package both depend on is ``kumo-connectors``, which is a SQL-connector
    package with no HTTP surface and no ``requests`` dependency, and
    ``kumo_relational_engine`` cannot import ``kumo_relational_client`` because the dependency runs the
...
class NimClient:
    def __init__(
        ...
        disables them. It is what ``RelationalClient(max_retries=...)`` forwards, so
