...
from kumo_relational_engine import in_tmux
from kumo_relational_engine.api.source_table import UnavailableSourceTable
...
class Table(ABC):
    ...
    def remove_column(self, name: str) -> Self:
        ...
        A :class:`~kumo_relational_engine.rfm.Table` does not know which graph holds it, so
        ...
        Call :meth:`~kumo_relational_engine.rfm.Graph.unlink` first when removing a linked
        foreign key; otherwise :meth:`~kumo_relational_engine.rfm.Graph.validate` rejects the
        graph.
