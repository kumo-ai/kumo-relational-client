...
logger = logging.getLogger('kumo_relational_engine_rfm')
...
class RfmGlobalState:
    ...
    def client(self) -> RFMTransport:
        ...
        return kumo_relational_engine.global_state.client
...
def _configure(
    ...
    global_state._url = kumo_relational_engine.global_state._url
...
def close_client(_token: object | None = None) -> None:
    ...
    if _token is not _CLIENT_TOKEN:
        raise RuntimeError(_DIRECT_USE_MESSAGE)
    with global_state._lock:
        thread_local = kumo_relational_engine.global_state.thread_local
...
def init_client(
    ...
    *,
    ...
    :attr:`~kumo_relational_engine.rfm.KumoRelational._api_client` resolves lazily -- at the first
    ...
    the global again later. Pass it to :class:`~kumo_relational_engine.rfm.KumoRelational` as
    ``_client``.
    """
    if _token is not _CLIENT_TOKEN:
        raise RuntimeError(_DIRECT_USE_MESSAGE)
...
def init_snowflake_serving(
    ...
    *,
    ...
    if _token is not _CLIENT_TOKEN:
        raise RuntimeError(_DIRECT_USE_MESSAGE)
    with global_state._lock:
        kumo_relational_engine.init_snowflake_serving(
            service,
            session=session,
            method=method,
            max_request_bytes=max_request_bytes,
