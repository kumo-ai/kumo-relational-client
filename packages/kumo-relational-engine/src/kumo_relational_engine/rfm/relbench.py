...
def _pooch() -> Any:
    ...
    :mod:`kumo_relational_engine.rfm` must not require the ``relbench`` extra.
    """
    try:
        ...
        raise ModuleNotFoundError(
            ...
            "'relbench' extra, e.g. `pip install 'kumo-relational-client[relbench]'` "
            "(or `pip install 'kumo_relational_engine[relbench]'`).",
            name='pooch',
        ) from error
...
def from_relbench(dataset: str, verbose: bool = True) -> Graph:
    ...
    Requires the ``relbench`` extra (``pip install 'kumo-relational-client[relbench]'``).
    """
