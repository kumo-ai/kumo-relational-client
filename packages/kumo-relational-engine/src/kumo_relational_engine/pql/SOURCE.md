# kumo_relational_engine.pql: provenance
...
- `kumopql.*` → `kumo_relational_engine.pql.*`, and `kumoapi.*` → `kumo_relational_engine.api.*`.
...
5. `parser/visitor.py`, a backtick-quoted name is a spelling device only, so
   it is canonicalized through `kumo_relational_engine._names.canonical_fqn` as the query is
