...
`scripts/generate_tfm_api.py`, the generated `kumo_relational_engine/client/generated/tfm_api.py`
...
- `kumo_relational_engine/client/rfm.py` row mapping vs parsed response fields
- `kumo_relational_engine/rfm/payload.py` request envelope vs `PredictionRequest`
...
Add request-side coverage for `kumo_relational_engine/rfm/payload.py`, which is handwritten and
