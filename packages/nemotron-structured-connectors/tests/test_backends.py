...
from nemotron_structured_connectors import connect
from nemotron_structured_connectors.backends import mark_owned, owns_connection
...
def test_databricks_driver_options_bypass_the_allow_list(monkeypatch):
    ...
    from nemotron_structured_connectors.backends import databricks as backend
...
def test_snowflake_driver_options_credentials_suppress_borrowing(monkeypatch):
    ...
    from nemotron_structured_connectors.backends import snowflake as backend
...
def test_snowflake_allows_named_connection_arguments(argument):
    ...
    snowflake = pytest.importorskip(
        'nemotron_structured_connectors.backends.snowflake'
    )
...
def test_databricks_allows_kwargs_routed_arguments(argument):
    ...
    databricks = pytest.importorskip(
        'nemotron_structured_connectors.backends.databricks'
    )
