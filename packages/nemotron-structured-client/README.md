...
```bash
pip install nemotron-structured-client              # client + every lightweight model (Nemotron Tabular)
pip install nemotron-structured-client[relational]     # adds Nemotron Relational (native driver)
pip install nemotron-structured-client[sqlite]      # data-source reads ([duckdb]/[snowflake]/[databricks]/[s3])
pip install nemotron-structured-client[all]         # Nemotron Relational, every data-source backend,
                                     # and [databricks-serving]
pip install nemotron-structured-client[explain]     # Nemotron Relational plus the explanation-summary LLM
...
A `StructuredClient` owns one connection to a NIM. Requests are typed per model: each
...
`StructuredError(code="INVALID_REQUEST")` before anything is sent. Everything else is
...
Nemotron Relational (relational), needs `nemotron-structured-client[relational]`:
...
> Unless `NEMOTRON_STRUCTURED_EXPLAIN_LLM_BASE_URL` points elsewhere, that endpoint is
> OpenAI's `https://api.openai.com/v1/` (model `gpt-4.1-mini-2025-04-14`, or
> `NEMOTRON_STRUCTURED_EXPLAIN_LLM_MODEL`), a non-NVIDIA service. The API key is read from
> `NEMOTRON_STRUCTURED_EXPLAIN_LLM_API_KEY` and nothing else. Setting that one
> variable is what turns the call on; a key exported for another tool, such as
> `OPENAI_API_KEY`, does not enable it.
>
> Nothing is sent if no key is discoverable or the `nemotron-structured-client[explain]` extra
...
`StructuredClient(url, timeout=30, max_retries=3)`. `timeout` bounds each attempt
...
through `StructuredClient` rather than mixing in direct `nemotron_relational.init()` calls.
`close()` (or leaving the `with` block) releases the pooled connections and
retires the client; use a new `StructuredClient` afterwards.

`from nemotron_structured import relational` is the supported surface onto the Nemotron Relational
...
Implement `ModelAdapter` in `nemotron_structured/adapters/<model>.py` with a typed
request, export it from `nemotron_structured/adapters/__init__.py`, and add it to
`nemotron_structured.client._default_registry`. Out of tree, build an `AdapterRegistry`
yourself and pass it as `StructuredClient(url, registry=...)`. Models needing a heavy
