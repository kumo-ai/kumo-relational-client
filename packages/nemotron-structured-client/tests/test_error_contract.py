...
Two things are being pinned: that ``except StructuredError`` catches everything the
...
from nemotron_structured.core.transport import _to_nim_error
from nemotron_structured.errors import MAX_INVALID_PARAMS, format_invalid_params
...
def test_error_types_are_exported(name: str) -> None:
    ...
    assert name in nemotron_structured.__all__
    assert hasattr(nemotron_structured, name)
...
@pytest.mark.parametrize(
    'cls',
    [StructuredError, NimRequestError, MissingExtraError, UnknownModelError],
)
