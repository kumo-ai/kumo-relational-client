...
from nemotron_structured.requests import NemotronTabularRequest

_ENV_VAR = 'NEMOTRON_STRUCTURED_NIM_BASE_URL'
...
def test_nim_reports_ready(client: StructuredClient):
    assert client.health_ready() is True


def test_tabicl_predict_returns_expected_shape(client: StructuredClient):
...
def test_tabicl_predict_regression_returns_quantiles(client: StructuredClient):
    context = pd.DataFrame(
        {
