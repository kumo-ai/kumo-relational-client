...
def test_predict_forwards_the_client_timeout_to_engine_init(monkeypatch):
    ...
    NemotronRelationalAdapter().predict(
        transport, NemotronRelationalRequest(graph='g', query='PREDICT x')
    )
...
def test_nemotron_relational_shim_does_not_expose_driver():
    from nemotron_structured import relational as nemotron_relational_shim
...
def test_nemotron_relational_shim_withholds_names_the_supported_api_cannot_reach():
    ...
    Neither is reachable through `StructuredClient`: the first is returned only by
...
def test_predict_rejects_explain_specified_twice(
    ...
    with pytest.raises(StructuredError) as err:
        NemotronRelationalAdapter().predict(
            client,
            NemotronRelationalRequest(
                graph='fake-graph',
...
def test_client_predict_explain_returns_explanation(monkeypatch):
    ...
    monkeypatch.setattr(
        'nemotron_structured.core.transport.Transport.health_ready',
        lambda self: True,
    )
    with StructuredClient(url='http://127.0.0.1:18001') as client:
...
def test_adapter_skips_batch_mode_when_unset(monkeypatch, client):
    ...
    NemotronRelationalAdapter().predict(
        client,
        NemotronRelationalRequest(graph='g', query='PREDICT x FOR t.id=1'),
    )
...
def test_nim_failure_becomes_a_nim_request_error(monkeypatch, client):
    ...
    with pytest.raises(NimRequestError) as excinfo:
        _predict(client)
    assert isinstance(excinfo.value, StructuredError)
...
def test_adapter_predict_error_is_not_rewrapped(monkeypatch, client):
    original = StructuredError('already ours', code='INVALID_REQUEST')
...
def test_capabilities_list_both_request_types():
    assert (
        ...
        == 'NemotronRelationalRequest | NemotronRelationalTaskRequest'
    )
...
def test_capabilities_advertise_supported_task_types():
    from nemotron_structured.adapters.relational import RFM_TASK_TYPES
...
def test_stype_and_dtype_belong_to_the_supported_namespace():
    ...
    # presents nemotron_structured.relational as the supported namespace. Membership is
...
def test_concurrent_clients_predict_against_their_own_endpoint(
    ...
    clients = {
        'A': StructuredClient(url='https://tenant-a.example', api_key='key-A'),
        'B': StructuredClient(url='https://tenant-b.example', api_key='key-B'),
    }
