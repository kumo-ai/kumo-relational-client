...
if TYPE_CHECKING:
    from nemotron_structured.base import PredictResult
...
def require_frame(value: Any, name: str) -> pd.DataFrame:
    ...
    adapter, which ``except StructuredError`` does not catch and which names neither
...
class RelationalModel:
    ...
    Returned by :meth:`StructuredClient.nemotron_relational`. It is a thin, stateless wrapper: the
        ...
        **engine_options: Any,
        ...
        Args:
            ...
            num_retries: Application-level retries of a failed prediction
                ...
                Distinct from
                ``StructuredClient(max_retries=...)``, which retries at the
                transport.
            ...
            engine_options: Any further keyword argument the engine's
                ...
                not accept raises ``StructuredError('INVALID_REQUEST')``.
...
class TabularModel:
    ...
    Returned by :meth:`StructuredClient.tabicl`. The context, task and target are
    ...
    def __del__(self) -> None:
        ...
        if session is None or session.id is None:
            return
        try:
            from nemotron_structured.adapters.tabular import (
                delete_session_quietly,
            )
