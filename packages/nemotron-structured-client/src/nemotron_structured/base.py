...
class ModelCapabilities:
    r"""What a model supports, discoverable via ``StructuredClient.capabilities(model)``.
...
class ModelAdapter(ABC):
    r"""The extension point for serving another model through ``StructuredClient``.
    ...
    is added to a client with ``StructuredClient._register``.
    """
    ...
    def predict(
        ...
        ``ServingTarget`` raises an ``StructuredError`` naming the reason from
        ...
        :class:`~nemotron_structured.adapters.relational.NemotronRelationalAdapter` does.
    ...
    def close(self) -> None:  # noqa: B027 - optional hook, see below
        ...
        Called by :meth:`~nemotron_structured.StructuredClient.close` for every registered
