...
from nemotron_structured.core.serving import (
...
from nemotron_structured.requests import ModelRequest


def _default_registry() -> AdapterRegistry:
    from nemotron_structured.adapters import (
...
class StructuredClient:
    ...
    Each ``StructuredClient`` owns its own transport and adapter registry, so several
    ...
    client configured it last. Do not mix ``StructuredClient`` with direct driver
    ...
    def _configure(
        ...
        r"""The one place an ``StructuredClient``'s fields are populated.
    ...
    def for_databricks_serving(
        ...
        *,
        ...
        Raises:
            StructuredError: with ``code='INVALID_CONFIGURATION'`` if ``endpoint`` is
        ...
        ambient configuration as ``StructuredError``.
        """
    ...
    def for_snowflake_serving(
        ...
        *,
        ...
        >>> client = StructuredClient.for_snowflake_serving("MYDB.MYSCHEMA.MY_MODEL_SVC")
        >>> df = client.relational(graph).predict("PREDICT ... FOR ...", [1, 2])
        ...
        Raises:
            StructuredError: with ``code='INVALID_CONFIGURATION'`` if ``service`` is
    ...
    def health_ready(self) -> bool:
        ...
        Raises:
            StructuredError: With code ``TRANSPORT_ERROR`` if the endpoint cannot be
                reached at all.
        """
    ...
    def close(self) -> None:
        ...
        Every later call raises; construct a new ``StructuredClient`` instead.
    ...
    def __repr__(self) -> str:
        ...
        return f'StructuredClient({where}, models={self.models()})'
