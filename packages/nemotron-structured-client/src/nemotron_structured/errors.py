...
Every call on :class:`~nemotron_structured.StructuredClient` and its model handles raises
:class:`StructuredError` or one of its subclasses, so ``except StructuredError`` is enough
...
def format_invalid_params(params: Sequence[Any] | None) -> str:
    ...
    Note:
        ...
        ``nemotron_structured``, and the package both do depend on is a SQL-connector
...
class UnknownModelError(StructuredError):
    r"""A request named a model no registered adapter serves.

    Raised by :meth:`~nemotron_structured.StructuredClient.capabilities` and when dispatching
...
class MissingExtraError(StructuredError):
    ...
    def __init__(self, extra: str, package: str) -> None:
        super().__init__(
            ...
            f'`pip install nemotron-structured-client[{extra}]`',
            code='MISSING_EXTRA',
            details={'extra': extra, 'package': package},
        )


class NimRequestError(StructuredError):
    r"""The NIM answered, and the answer was an error.

    Distinguished from :class:`StructuredError` by carrying the HTTP
