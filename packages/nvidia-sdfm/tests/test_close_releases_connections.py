...
r"""What ``SDFMClient.close()`` has to release.

The client owns one pool, its transport. A model backed by a driver does not
send through that pool: the KumoRFM driver opens its own, so closing the
...
def test_two_clients_close_independently() -> None:
    a, b = SDFMClient(url='http://a.test'), SDFMClient(url='http://b.test')
...
def test_kumorfm_adapter_close_is_inert_before_any_prediction() -> None:
    ...
    adapter = KumoRFMAdapter()
    assert not adapter._opened_engine_client
    adapter.close()


def test_kumorfm_adapter_releases_the_engine_it_opened(monkeypatch) -> None:
    from nvidia_sdfm.adapters import kumorfm as adapter_module
    ...
    adapter = adapter_module.KumoRFMAdapter()
...
def test_kumorfm_adapter_does_not_release_twice(monkeypatch) -> None:
