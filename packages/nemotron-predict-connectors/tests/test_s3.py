...
from nemotron_predict_connectors import read
from nemotron_predict_connectors.sql import ConnectorError, MissingBackendError

s3fs = pytest.importorskip('s3fs')
moto_server = pytest.importorskip('moto.server')

BUCKET = 'nemotron-predict-connectors-test'
...
def test_read_s3_missing_driver_raises(monkeypatch):
    ...
    assert 'nemotron-predict-connectors[s3]' in str(excinfo.value)


@pytest.mark.skipif(
    'NEMOTRON_PREDICT_S3_LIVE_URI' not in os.environ,
    reason='NEMOTRON_PREDICT_S3_LIVE_URI not set',
)
def test_read_s3_live():
    anon = os.environ.get('NEMOTRON_PREDICT_S3_LIVE_ANON', '').lower()
    options = {'anon': True} if anon in ('1', 'true', 'yes') else None
    frame = read(
        's3',
        path=os.environ['NEMOTRON_PREDICT_S3_LIVE_URI'],
        storage_options=options,
    )
    assert len(frame) > 0
