...
nemotron-predict-client's `DRIVER_LOAD_FAILED`).

Pure-python. Database drivers are optional extras:

```bash
pip install "nemotron-predict-connectors[sqlite]"      # or [duckdb] / [snowflake] / [databricks] / [s3] / [all]
```

Consumers depend on it and surface these extras under their own name, e.g.
`nemotron-predict-client[snowflake]` and `nemotron_relational[databricks]` both resolve the corresponding
`nemotron-predict-connectors` extra.
